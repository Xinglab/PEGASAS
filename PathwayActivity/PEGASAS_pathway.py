...
parser = argparse.ArgumentParser(description='PEGASAS-Pathway (v1.0)')
parser.add_argument('geneExpbySample',help='A TSV format matrix of gene expression values (FPKM, TPM, etc.) where each column is one sample and each row is one gene.')
parser.add_argument('geneSignatureList',help='One or multiple gene signature sets of pathway of interest in the format of \'gmt\' (see MSigDB webset).')
parser.add_argument('groupInfo',help='A TSV format file provides patient ID and phenotype/disease stage in each row.')
parser.add_argument('-o','--out-dir', default='PEGASAS_PathwayScore', help='Output folder name of the analysis.')
parser.add_argument('-n','--num-interval',default=40, help='Number of KS enrichment calculation processes one time.')
parser.add_argument('--plotting',default=False, action='store_true', help='Making plots to inspect K-S enrichment scores.')
...
for gs in geneSet:
	...
	os.system('cat '+score_outdir.rstrip('/')+'/*.txt >'+score_outdir.rstrip('/')+'.scores.txt')
